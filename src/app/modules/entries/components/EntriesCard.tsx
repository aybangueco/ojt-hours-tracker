...
import { actionDeleteEntry, actionUpdateEntry, useEntryContext } from "../";
...
export default function EntriesCard({
	...
	const { setTimeEntries, timeEntries } = useEntryContext();
	...
	const handleUpdateEntry = async (id: number) => {
		...
		const { ok } = await actionUpdateEntry(id, authUser.id, entryValue);
	...
	const handleDeleteEntry = async (id: number) => {
		...
		const { ok } = await actionDeleteEntry(id, authUser.id);
	...
	return (
		<div>
			<div key={entry.id} className="border rounded-lg p-4">
				...
				{index < timeEntries.length - 1 && <Separator className="mt-4" />}
			</div>
		</div>
	);
}
